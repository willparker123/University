...
void handleInput(game *g, char *coord)
{
  int r = row(coord);
  int c = col(coord);
  move(g,r,c);
...
void ask(game *g, char player)
{
  char *coord = malloc(sizeof(char)*3);
  printf("It is '%c's turn: ", show(player));
  scanf(" %s", coord);
  if (valid(g,coord)!=OK)
  {
    printInvalid(valid(g,coord));
    ask(g,player);
  }
  else handleInput(g,coord);
}

void playLogic(game *g, char player)
{
  while (draw(g)==false && winCheck(g)==false) //when game is not over
  {
    ask(g, player);
...
{
    if (n == 2 && strcmp(args[1],"X") == 0) play(X);
    else if (n == 2 && strcmp(args[1],"O") == 0) play(O);
    else {
        fprintf(stderr, "Use: ./oxo X  OR  ./oxo O based on the player who is to start.\n");
        exit(1);
    }
